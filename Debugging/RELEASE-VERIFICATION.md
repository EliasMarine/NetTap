...
> **Status:** 16/17 checks verified across 2 environments (Dev + N100) — ALL automated checks PASS. Hardware checks (H1–H7) in progress. **H1 near-complete — 18/18 containers healthy.** Full-stack test (full-stack-test.sh) 59/59 passing — covers all Mirror/SPAN API endpoints + web UI pages. Log Explorer fully fixed: .keyword suffix, 10K cap, chart flicker, colors, alerts/pcap-monitor noise exclusion. Design system created + 10 pages standardized. TShark filter fixed (ephemeral port, ICMP, IPv6). Deploy script hardened (OpenSearch health check). Web tests: 1098/1098 passing. svelte-check: 0 errors.
> **Target:** v1.0.0
...
| 7 | Web Unit Tests (vitest) | PASS | PASS | SKIP | SKIP | — | 1098/1098 passed on Dev (was 691; +407 from Log Explorer, TShark, Design System, SIEM features) |
...
| 2026-03-10 | N100 (Ubuntu) | Data pipeline verification | **FIXED** | — | — | — | Elias | Two root causes fixed: (1) `ZEEK_JSON` env var missing → Zeek output TSV instead of JSON → Logstash couldn't parse → conn/dns/http/tls logs lost. Fix: added `ZEEK_JSON: "true"` (commit 5d13793). (2) `event.id` field type conflict: arkime_sessions3-260310 created by Suricata first (numeric event.id → mapped as `long`). Zeek string UIDs rejected with mapper_parsing_exception. Fix: deleted conflicted index + updated malcolm_template to map `event.id` as `keyword`. Result: 7,427+ Zeek conn docs flowing. Dashboard populating. |
| 2026-03-09 | Dev (macOS) | Mirror/SPAN proxy route fix | **FIXED** | 1034 | 0 | 0 | Claude | 7 Mirror/SPAN feature pages (Live Monitor, Bandwidth, DNS Analytics, IoT & LAN, Changelog, Certificates, PCAP Search) showed no data. Root cause: missing SvelteKit `+server.ts` proxy routes (404 silently) + `VITE_API_URL` pointing to Docker-internal port 8880 (unreachable from browser). Fix: catch-all `[...path]/+server.ts` proxy via `daemonFetch()` + removed `VITE_API_URL` from 5 API clients + setup page. 8 new tests. vitest: 1034/1034 (74 files). svelte-check: 0 errors. |
| 2026-03-09 | Dev (macOS) | PCAP Search fixes | **FIXED** | -- | 0 | 0 | Claude | Quick filters used BPF syntax (`udp port 53`) but tshark `-Y` requires Wireshark display filter syntax (`dns`). Filter validation regex blocked `\|`/`&` needed for display filter `\|\|`/`&&` — removed from forbidden chars (safe: `asyncio.create_subprocess_exec` prevents shell injection). Added `GET /api/pcap/download-file` daemon endpoint + Download buttons in UI. Added auto-scroll to preview section + sortable columns. Files: `daemon/api/pcap.py`, `daemon/services/pcap_search.py`, `web/src/lib/api/pcap.ts`, `web/src/routes/pcap/+page.svelte`. Commits 6a853e4 + 8763d07. |
| 2026-03-09 | Dev (macOS) | DNS Analytics fixes | **FIXED** | -- | 0 | 0 | Claude | All DNS aggregations returned 0 due to field name mismatch: `dns.question.name.keyword` → `zeek.dns.query.keyword`, `dns.question.type.keyword` → `zeek.dns.qtype_name.keyword`, `dns.response_code.keyword` → `zeek.dns.rcode_name.keyword`, `event.duration` → `zeek.dns.rtt`. RTT conversion inverted: was ÷1,000,000 (assuming nanoseconds), now ×1000 (Zeek stores seconds). Complete page redesign: interactive SVG timeline with tooltips, sortable tables, time range pills, per-device DNS split panel, empty states. Files: `daemon/services/dns_analytics.py`, `web/src/routes/dns/+page.svelte`. Commit 5e8acf8. |
| 2026-03-09 | Dev (macOS) | PCAP Search redesign + DNS copy fix | **DONE** | -- | 0 | 0 | Claude | Full PCAP Search page redesign with interactive features matching DNS Analytics quality. Hero stats, capture timeline SVG chart, quick filter chips, sortable search results, packet preview panel, protocol breakdown, two-column file browser, cross-section linking, time range pills. DNS copy button CSS fix (1.25rem, accent-blue). Added `formatRelativeTime` + `PROTO_FILTER_MAP` helpers to `pcap.ts`. Files: `web/src/routes/pcap/+page.svelte`, `web/src/routes/dns/+page.svelte`, `web/src/lib/api/pcap.ts`. Commit c02cb9f. |
| 2026-03-09 | Dev (macOS) | Alerts page redesign | **DONE** | 1053 | 0 | 0 | Claude | Full Alerts page redesign with 4 new backend aggregation endpoints and interactive frontend. Backend: `/api/alerts/timeline` (date_histogram), `/api/alerts/top-signatures` (terms agg), `/api/alerts/top-ips` (src/dest), `/api/alerts/categories`. 8 total endpoints. Frontend: hero stats, SVG timeline chart with hover tooltips, two-col signatures+categories, two-col attacked+source IPs with copy buttons, sortable table, severity filter pills, time range pills, Promise.allSettled loading, initialized guard. API client: 4 new fetch functions, 7 new types, 3 formatting helpers. Tests: 18 new (25 total). 4 new SvelteKit proxy routes. svelte-check: 0 errors. vitest: 1053/1053. Files: `daemon/api/alerts.py`, `web/src/lib/api/alerts.ts`, `web/src/lib/api/alerts.test.ts`, `web/src/routes/alerts/+page.svelte`, 4 proxy routes. Commit 4009faf. |
| 2026-03-10 | Dev (macOS) | Log Explorer fixes (6 commits) | **FIXED** | 1098 | 0 | 0 | Claude | 6 commits: .keyword suffix on 6 agg fields (1f85e8e), 10K event cap + text color + SVG flicker + chart colors (6c70af4), Suricata alerts exclusion (245d4af), pcap-monitor noise exclusion from search (8e09867). svelte-check: 0 errors, 9 warnings. vitest: 1098/1098 (76 files). |
| 2026-03-10 | Dev (macOS) | Design system + 10-page standardization | **DONE** | -- | 0 | 0 | Claude | Created `web/DESIGN-SYSTEM.md` canonical design reference. Standardized 10 pages: removed 110+ hardcoded hex colors, spacing values, font sizes, max-width constraints. Added sortable tables to certificates (6 cols), infrastructure (3 tabs), dashboard (alerts). Updated CLAUDE.md with mandatory design system section. Commit 0e10121. |
| 2026-03-10 | Dev (macOS) | TShark filter fix | **FIXED** | 1098 | 0 | 0 | Claude | `buildTSharkFilter()` was including ephemeral source port making filter unmatchable across Arkime's rotating PCAPs. Also missing ICMP (no ports) and IPv6 (`ipv6.addr`) handling. Extracted to `web/src/lib/utils/tshark-filter.ts` utility module with 18 tests covering TCP/UDP/ICMP/ICMPv6/IPv4/IPv6/mixed/arrays. svelte-check: 0 errors. vitest: 1098/1098. NET-110, commit 7912342. |
| 2026-03-11 | Dev (macOS) | Deploy script hardening | **DONE** | -- | 0 | 0 | Claude | Deploy script (`deploy-log-explorer.sh`) now checks OpenSearch health before deploying daemon/web. If unhealthy: restart + wait 180s + security bootstrap as fallback. Prevents `dependency failed to start: container nettap-opensearch is unhealthy` error. Commit 1ce47e3. |
| 2026-03-11 | N100 (Ubuntu) | Deploy with OpenSearch unhealthy | **FAILED** | -- | 1 | 0 | Elias | `deploy-log-explorer.sh` hit `dependency failed to start: container nettap-opensearch is unhealthy`. Old script had no OpenSearch health check. Fixed in 1ce47e3. Pending: re-run with updated script. |
