...
> **Status:** 16/17 checks verified across 2 environments (Dev + N100) — ALL automated checks PASS. Hardware checks (H1–H7) in progress. **H1 near-complete — all 20 containers healthy, nuclear test (`down -v && up -d`) PASS: 34/34 steps, logstash 64.9s, zero manual intervention.** OpenSearch security auto-bootstrap permanently fixed (targeted `-f/-t` push + shared certs volume + Docker DNS + stub template). BATS compose validation: 36/37 pass (7 new tests for init service). Test counts: pytest 1041, vitest 683, BATS 37.
> **Target:** v1.0.0
...
|---|---|---|---|---|---|---|
| H1 | Integration test — full Docker stack up | N100 | **NEAR COMPLETE** | 2026-03-04 | Elias | **All 20 containers healthy.** Cluster GREEN (24/24 shards). Dashboard showing "bandwidth over time" live data. Init container pushes rolesmapping on attempt 1, auth 200 immediately, logstash healthy at 64.9s. Setup wizard completes end-to-end (Steps 1-5 including account creation). System page works (null SMART values handled). SSE notification stream working through nginx. **Nuclear test passed:** `down -v && up -d` — 34/34 steps, stub template breaks logstash deadlock, all 20 containers up in ~65s, zero manual intervention. **Issues fixed:** NET-82 (CSRF), NET-83 (null SMART), NET-85/86 (healthchecks), Chain 11a/11b/11d (init container `-cd` overwrite + network namespace + shared certs volume + stub template deadlock). **Known issue:** nginx-proxy unhealthy — Malcolm's nginx references `arkime:8005` upstream but arkime-live uses host networking (invisible to Docker DNS). Not critical — nettap-nginx handles all user traffic. **Remaining:** Verify data pipeline end-to-end (traffic through bridge → Zeek/Suricata → OpenSearch → dashboard). |
...
| 2026-03-04 | N100 (Ubuntu) | H1: Init container debug | **PASS** | — | — | — | Elias | Fixed init container: `-cd` → `-f/-t` (stops internal_users overwrite), added `-h opensearch -p 9200` (Docker DNS), added `opensearch-certs` shared volume (TLS certs), `entrypoint: ["/bin/bash"]` (skip Malcolm chain). Results: rolesmapping SUCC attempt 1, auth HTTP 200 immediately, cluster GREEN 24/24 shards, logstash healthy 60.6s, all 20 containers up with checkmarks, dashboard showing "bandwidth over time" data. Commits: 90b4e38, 7014cda. |
| 2026-03-04 | N100 (Ubuntu) | H1: Nuclear test (`down -v && up -d`) | **PASS** | — | — | — | Elias | Full fresh install scenario — `docker compose down -v` then `up -d`. Stub `malcolm_template` fix (commit 733672e) breaks logstash/dashboards-helper circular deadlock. Results: 34/34 docker compose steps completed with checkmarks. Init container: roles_mapping SUCC → auth HTTP 200 → stub template created → done in 39.3s. Logstash healthy at 64.9s (was deadlocked 659s+ before fix). Filebeat started at 65.0s. All 20 containers up, zero manual intervention. Complete fresh install scenario confirmed working. |
