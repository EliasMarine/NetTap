...
EXTEOF
    ...
    # Create nginx-proxy compatible copies.
    # Malcolm's nginx-proxy expects cert.pem/key.pem (not server.crt/server.key).
    # The entrypoint auto-generates these if missing, but our mount is :ro so it
    # can't write. We must provide the correct filenames.
    cp "${certs_dir}/server.crt" "${certs_dir}/cert.pem"
    cp "${certs_dir}/server.key" "${certs_dir}/key.pem"

    # Generate DH parameters for nginx-proxy TLS (dhparam.pem).
    # Malcolm's nginx-proxy expects this file. Auto-generation fails on :ro mount.
    if [[ ! -f "${certs_dir}/dhparam.pem" ]]; then
        log "Generating DH parameters (this may take a moment)..."
        run openssl dhparam -out "${certs_dir}/dhparam.pem" 2048
    fi

    # Set permissions
    chmod 600 "${certs_dir}"/*.key "${certs_dir}/key.pem"
    chmod 644 "${certs_dir}"/*.crt "${certs_dir}/cert.pem" "${certs_dir}/dhparam.pem"
...
# ---------------------------------------------------------------------------
# Generate TLS certificate for the NetTap nginx reverse proxy
# ---------------------------------------------------------------------------
# nettap-nginx (our custom nginx) is separate from Malcolm's nginx-proxy.
# It expects nettap.crt/nettap.key at /etc/nginx/ssl/ (mounted from docker/ssl/).
# We reuse the CA from generate_certs() to sign this cert.
generate_nginx_ssl() {
    local ssl_dir="${PROJECT_ROOT}/docker/ssl"
    local ca_dir="${PROJECT_ROOT}/docker/certs"

    if [[ -f "${ssl_dir}/nettap.crt" && -f "${ssl_dir}/nettap.key" ]]; then
        log "NetTap nginx SSL cert already exists in ${ssl_dir}, skipping"
        return 0
    fi

    # CA must exist first (generated by generate_certs)
    if [[ ! -f "${ca_dir}/ca.crt" || ! -f "${ca_dir}/ca.key" ]]; then
        error "CA certificate not found in ${ca_dir}. Run generate_certs() first."
    fi

    log "Generating TLS certificate for NetTap nginx..."
    mkdir -p "$ssl_dir"

    # Generate private key
    run openssl genrsa -out "${ssl_dir}/nettap.key" 2048

    # Create CSR
    run openssl req -new \
        -key "${ssl_dir}/nettap.key" \
        -out "${ssl_dir}/nettap.csr" \
        -subj "/C=US/ST=NetTap/L=Local/O=NetTap/OU=WebUI/CN=${NETTAP_HOSTNAME:-nettap.local}"

    # SAN extension — include the hostname, localhost, and common LAN patterns
    cat > "${ssl_dir}/nettap-ext.cnf" <<EXTEOF
authorityKeyIdentifier=keyid,issuer
basicConstraints=CA:FALSE
keyUsage = digitalSignature, nonRepudiation, keyEncipherment, dataEncipherment
subjectAltName = @alt_names
...
[alt_names]
DNS.1 = ${NETTAP_HOSTNAME:-nettap.local}
DNS.2 = localhost
DNS.3 = nettap
DNS.4 = nettap-nginx
IP.1 = 127.0.0.1
EXTEOF
    ...
    run openssl x509 -req \
        -in "${ssl_dir}/nettap.csr" \
        -CA "${ca_dir}/ca.crt" \
        -CAkey "${ca_dir}/ca.key" \
        -CAcreateserial \
        -out "${ssl_dir}/nettap.crt" \
        -days 3650 \
        -sha256 \
        -extfile "${ssl_dir}/nettap-ext.cnf"

    # Clean up temp files
    rm -f "${ssl_dir}/nettap.csr" "${ssl_dir}/nettap-ext.cnf" "${ca_dir}/ca.srl"

    # Permissions: key is sensitive, cert is public
    chmod 600 "${ssl_dir}/nettap.key"
    chmod 644 "${ssl_dir}/nettap.crt"

    log "NetTap nginx SSL certificate generated in ${ssl_dir}"
}
...
configure_malcolm() {
    ...
    log "  Nginx SSL:    ${PROJECT_ROOT}/docker/ssl/"
